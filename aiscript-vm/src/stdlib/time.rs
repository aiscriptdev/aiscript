...
fn time_now<'gc>(_mc: &'gc Mutation<'gc>, _args: Vec<Value<'gc>>) -> Result<Value<'gc>, VmError> {
...
fn time_sleep<'gc>(_mc: &'gc Mutation<'gc>, args: Vec<Value<'gc>>) -> Result<Value<'gc>, VmError> {
...
fn time_to_utc<'gc>(mc: &'gc Mutation<'gc>, args: Vec<Value<'gc>>) -> Result<Value<'gc>, VmError> {
    ...
    Ok(Value::IoString(Gc::new(mc, formatted)))
}
...
fn time_hours<'gc>(_mc: &'gc Mutation<'gc>, args: Vec<Value<'gc>>) -> Result<Value<'gc>, VmError> {
    let hours = float_arg!(&args, 0, "hours")?;
    Ok(Value::Number(hours * 3600.0))
}

fn time_days<'gc>(_mc: &'gc Mutation<'gc>, args: Vec<Value<'gc>>) -> Result<Value<'gc>, VmError> {
